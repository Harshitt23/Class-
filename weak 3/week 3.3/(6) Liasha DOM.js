...
// ------------------------------------------------------------------
// Runnable DOM examples below. (The original notes are preserved above)
// Save this file and open the examples in the browser (or copy them into an HTML file)
// Example 1: Simple greeting (works if put into an HTML file or run via DOM in browser)

/*
To run Example 1 in browser:
1) Create a file greeting.html with the HTML content below
2) Open greeting.html in your browser
*/
...
<body>
  <label for="nameInput">Enter Your Name:</label>
  <input type="text" id="nameInput">
  <button id="greetBtn">Submit</button>
  <p id="greetingMessage"></p>
...
// Example 2: Change input to button dynamically (snippet to paste inside an HTML file)
const changeInputHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Change Input to Button</title>
</head>
<body>
  <input type="text" id="myInput" value="Type Something">
  <button id="changeBtn">Change to Button</button>
  ...
  <script>
    ...
    document.getElementById('changeBtn').addEventListener('click', changeToButton);
  <\/script>
</body>
</html>`;

// Example 3: Event listeners demo (paste into HTML file to run)
const eventsDemoHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Event Listeners Demo</title>
  <style>#myButton{padding:10px;font-size:16px;cursor:pointer;}</style>
</head>
<body>
  <button id="myButton">Click me</button>
  <div id="outer" style="border:1px solid red;padding:10px;margin-top:10px">Outer
    <div id="middle" style="border:1px solid green;padding:10px;">Middle
      <div id="inner" style="border:1px solid blue;padding:10px;">Inner</div>
    </div>
  </div>

  <script>
    var button = document.getElementById('myButton');
    button.addEventListener('click', function() { button.style.backgroundColor = 'green'; alert('Button clicked!'); });
    button.addEventListener('mouseenter', function() { button.style.backgroundColor = 'yellow'; });
    button.addEventListener('mouseleave', function() { button.style.backgroundColor = ''; });

    document.getElementById('outer').addEventListener('click', function(){ console.log('Outer Bubbling'); });
    document.getElementById('middle').addEventListener('click', function(){ console.log('Middle Bubbling'); });
    document.getElementById('inner').addEventListener('click', function(){ console.log('Inner Bubbling'); });

    // Capturing example
    document.getElementById('outer').addEventListener('click', function(){ console.log('Outer Capturing'); }, true);
    document.getElementById('middle').addEventListener('click', function(){ console.log('Middle Capturing'); }, true);
    document.getElementById('inner').addEventListener('click', function(){ console.log('Inner Capturing'); }, true);
  <\/script>
</body>
</html>`;

// Helper: write the three HTML examples to files for you to open directly.
const fs = require('fs');
fs.writeFileSync('/mnt/data/dom_greeting.html', greetingHtml);
fs.writeFileSync('/mnt/data/dom_change_input.html', changeInputHtml);
fs.writeFileSync('/mnt/data/dom_events_demo.html', eventsDemoHtml);

console.log('DOM_notes.js created. Also wrote 3 example HTML files to /mnt/data:\n - dom_greeting.html\n - dom_change_input.html\n - dom_events_demo.html');
